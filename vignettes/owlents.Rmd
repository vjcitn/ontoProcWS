...
```{r lk1, message=FALSE}
BiocManager::install(c("reticulate", "ontoProc", "vjcitn/ontoProcWS",
  "vjcitn/gwasCatSearch"), ask=FALSE, update=FALSE) # innocuous, for use with AnVILPublish
library(ontoProcWS) # important for configuration process
library(ontoProc)
...
```{r doop}
library(reticulate)
py_require("owlready2")  # not always persistent...
...
bad = grep("STATO_0000416", tmp)  # see https://github.com/obophenotype/cell-ontology/issues/3237
# 1:2 because need to kill comment and then use
if (length(bad)>0) tmp = tmp[-bad[1:2]]
bad = grep("STATO_0000663", tmp)  # see https://github.com/obophenotype/cell-ontology/issues/3237
if (length(bad)>0) tmp = tmp[-bad[1:2]]
tf = tempfile()
writeLines(tmp, tf)
